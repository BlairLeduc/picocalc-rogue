...
diff -u modules/rogue/rip.c working/rip.c >> patches/rogue.patch
...
diff -u modules/PDCurses/pdcurses/getch.c working/getch.c > patches/pdcurses.patch
sed -i '' -E 's/^(---) (modules\/PDCurses\/pdcurses|working)\/([a-z_]+\.[ch])/\1 a\/pdcurses\/\3/; s/^(\+\+\+) (modules\/PDCURSES\/pdcurses|working)\/([a-z_]+\.[ch])/\1 b\/pdcurses\/\3/' patches/pdcurses.patch
